...
| 제약 | 이유 |
| :--- | :--- |
| **이모지·이모티콘을 UI에 넣지 않는다 — 예외는 카테고리 마크뿐** | 사이드바·카드·헤딩은 letter-mark로 통일한다. 카테고리 마크만 `CATEGORY_ICON`(`src/lib/category-meta.ts`)에 이모지를 매핑해 쓴다. 매핑이 없는 카테고리는 이름 첫 글자로 폴백된다. **이 예외를 다른 UI로 넓히지 마라** |
...
- **Pink `--mark: #d81b60`는 마크에만** — 아바타, 레터마크(사이드바 활성·카테고리 카드·글 카드 폴백), SVG 아이콘, 홈 섹션 카운터(`01`/`02`), 시리즈 도트·타임라인 노드. 배경 틴트는 `--mark-tint`. **읽는 텍스트에는 절대 쓰지 않는다** — 링크·본문·칩·TOC 는 블루가 정본이다. 두 색이 같은 요소에서 겹치면 안 된다: 요소마다 "글자냐 마크냐"로 하나만 고른다
- **그림자를 쓰지 않는다**
