...
표지는 frontmatter 로 지정한다. 본문 이미지와 **같은 경로 규칙**이다.
...
| 위치 | 없을 때 |
| :--- | :--- |
| 글 상세 페이지 **제목 위 표지 배너** | 배너 없이 제목부터 시작 |
| 홈의 히어로·사이드 카드 썸네일 | 카테고리 첫 글자 letter-mark + 그라데이션 |

굳이 넣지 않아도 된다. 배너는 가로로 잘리므로(데스크톱 3:1, 640px 이하 2:1) **가로로 넓은 이미지**가 잘 맞고, 세로가 긴 이미지는 위아래가 잘린다.
